...
执行命令时要注意：DeepAgents 原生 `execute` 默认已经在 `/projects` 对应的本地目录下执行，所以命令中的仓库路径直接写 `<repo>`，例如 `git -C ai_coding status`。不要在 `execute` 命令中再写 `projects/<repo>`，否则会变成错误的 `projects/projects/<repo>`。
