...
这个文件是 FastAPI 版本 LX-AICODING 的任务调度中心，主要负责把一次用户输入
...
讲课时可以把 runtime.py 理解成“产品流程控制层”：它保证用户没有确认方案前不会
...
def initialize_task_record(
    *,
    ...
    record_event(thread_id, "created", "任务已创建", status="completed")  # 存入store数据库
    return thread_id
...
def run_pull_only_task(*, repo_url: str, prompt: str, thread_id: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if target.exists() and (target / ".git").exists():
            ...
            # 这里使用 --ff-only，避免自动产生 merge commit，保持教学项目历史清晰。
            ...
            # 课程版目录固定为 projects/<repo>，不再写入仓库目录映射表。
            record_event(thread_id, "workspace", "准备本地工作区", status="completed")
